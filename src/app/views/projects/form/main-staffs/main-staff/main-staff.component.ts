...
export class MainStaffComponent extends AbstractForm implements OnInit {
  ...
  loadSubFaculty(event) {
    this.subFacultyLoaded.emit({ faculty_id: event.target.value, index: this.index });
  }

  loadBranch(event) {
    this.branchLoaded.emit({ sub_faculty_id: event.target.value, index: this.index });
  }

  loadStaff(event) {
    this.staffLoaded.emit({ branch_id: event.target.value, index: this.index });
  }
