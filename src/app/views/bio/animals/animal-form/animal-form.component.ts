...
export class AnimalFormComponent implements OnInit {
  ...
  formErrors = { name: '', group: '' };
  validationMessages = {
    name: {
      required: 'กรุณาระบุชื่อสัตว์'
    },
    group: {
      required: 'กรุณาเลือกกลุ่มของสัตว์'
    }
  };
  ...
  submit() {
    ...
    this.bioService.storeBio(this.form.value, 2).subscribe(
      res => {
  ...
  loadData() {
    this.bioService.getAnimalGroups().subscribe(data => this.animalGroups = data);
  }
