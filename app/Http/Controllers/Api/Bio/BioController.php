...
class BioController extends Controller
{
  public function index(Request $request) {
    ...
    $plants = BioMasPlants::get(['id','lat','lng', 'bio_trn_bio_file_group_id']);
    $animals = BioMasAnimals::get(['id','lat','lng', 'bio_trn_bio_file_group_id']);

    foreach($plants as $plant) {
      $plant->image = BioTrnBioFiles::where('file_group_id', $plant->bio_trn_bio_file_group_id)->first();
    }
    foreach($animals as $animal) {
      $animal->image = BioTrnBioFiles::where('file_group_id', $animal->bio_trn_bio_file_group_id)->first();
    }

    return response()->json(compact('plants', 'animals'));
  ...
  public function countAnimals() {
    $count = BioMasAnimals::where('sys_user_id', auth('api')->user()->id)->count();

    return response()->json($count);
  }

  public function countPlants() {
    $count = BioMasPlants::where('sys_user_id', auth('api')->user()->id)->count();
